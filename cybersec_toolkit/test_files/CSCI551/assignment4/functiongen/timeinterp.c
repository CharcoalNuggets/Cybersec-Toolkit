...
int main(int argc, char *argv[])
{
    ...
    FILE *fp = fopen("timeinterp_output.csv", "w");
    if (!fp) {
        perror("Error opening file");
        return 1;
    }

    fprintf(fp, "time,interpolated_accel,original_accel\n");
    long unsigned profile_size = sizeof(DefaultProfile) / sizeof(double);

    for(idx = 0; idx <= (steps_per_sec*1800); idx++){
        time = (dt * (double)idx);
        double interpolated_accel = faccel(time);
        double original_accel = (idx % steps_per_sec == 0 && (idx / steps_per_sec) < profile_size) ? table_accel(idx / steps_per_sec) : 0.0;
        fprintf(fp, "%015.14lf,%015.14lf,%015.14lf\n", time, interpolated_accel, original_accel);
    }

    fclose(fp);
    printf("CSV file 'timeinterp_output.csv' generated successfully.\n");
    return 0;
