...
#define IMG_HEIGHT (960)//need to pay attention to height vs width. Mixed it up and caused the technicolor effect   -dg
...
{
    int fdin, fdout, bytesRead=0, bytesWritten=0, bytesLeft, i, j, iter, rc, pixel, readcnt=0, writecnt=0, it;
    UINT64 microsecs=0, millisecs=0;
    FLOAT temp, fstart, fnow;
    struct timespec start, now;
    int thread_count=12;//changed thread count to 12 instead of 16 for comparison with sharpen_grid.c   -dg

for(it = 0; it < 5; it++){//repeat 5 times  -dg
...
#pragma omp parallel for num_threads(thread_count)//pragma directive for parallelism already here   -dg
